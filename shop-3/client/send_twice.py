...
# send_twice.py
# -------------
# Sends the same message twice, with the same message_id
...
def send_data_double(url, data):

  reply = requests.post(url, json.dumps(data))
  print ("Reply(1)       = " + str(reply))
  reply = requests.post(url, json.dumps(data))
  print ("Reply(2)       = " + str(reply))
...
response       = send_data_double(url, data)
